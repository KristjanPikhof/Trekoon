...
function buildSessionReadiness(domain: TrackerDomain): {
  readyCount: number;
  blockedCount: number;
  nextTaskId: string | null;
  nextTaskBlockers: DependencyBlocker[];
} {
  const openStatuses = new Set(["in_progress", "in-progress", "todo"]);
  const openTasks = domain.listTasks(undefined).filter((task) => openStatuses.has(task.status));
  ...
  const assessed = openTasks
    .map((task) => {
      ...
      for (const dep of dependencies) {
        const depStatus =
          dep.dependsOnKind === "task"
            ? domain.getTaskOrThrow(dep.dependsOnId).status
            : domain.getSubtaskOrThrow(dep.dependsOnId).status;

        if (depStatus !== "done") {
          blockers.push({
            id: dep.dependsOnId,
            kind: dep.dependsOnKind,
            status: depStatus,
          });
        }
      }
    ...
    .sort((left, right) => {
      ...
      if (byBlockers !== 0) return byBlockers;
  ...
  const readyCandidates = assessed.filter((item) => item.isReady);
  const blockedCandidates = assessed.filter((item) => !item.isReady);
  const top = readyCandidates[0] ?? null;

  return {
    readyCount: readyCandidates.length,
    blockedCount: blockedCandidates.length,
    nextTaskId: top?.task.id ?? null,
    nextTaskBlockers: top?.blockers ?? [],
  };
}
