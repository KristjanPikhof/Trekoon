...
describe("migration 0012: dependency kind indexes", (): void => {
  ...
  test("dedupe DELETE is skipped on a clean DB (no warn emitted)", (): void => {
    // A freshly-migrated DB has no duplicate dependency edges; the EXISTS
    // guard must prevent the window-function DELETE from running, and no
    ...
    let storage;
    try {
      storage = openTrekoonDatabase(workspace);
    } finally {
      console.warn = originalWarn;
    }

    try {
      const migrationWarns = warnMessages.filter((msg) =>
        msg.includes("0012_dependency_kind_indexes"),
      );
  ...
  test("dedupe DELETE still runs on a dirty DB (warn emitted)", (): void => {
    // Simulate a pre-0012 DB with duplicate dependency edges, then
    // re-apply v12. The EXISTS guard must detect the duplicates and allow
    // the DELETE + warn to run.
    const workspace: string = createWorkspace();
    const storage = openTrekoonDatabase(workspace);

    try {
      // Roll the schema back to v11 and stage duplicates.
      storage.db.exec("DELETE FROM schema_migrations WHERE version >= 12;");
      storage.db.exec("DROP INDEX IF EXISTS uniq_dependencies_edge;");
      storage.db.exec("DROP INDEX IF EXISTS idx_dependencies_target;");
      storage.db.exec("DROP INDEX IF EXISTS idx_dependencies_source;");
      ...
      storage.db
        ...
        .run("dup-a", "src-1", "task", "tgt-1", "task", base, base);
      storage.db
        .query(
          "INSERT INTO dependencies (id, source_id, source_kind, depends_on_id, depends_on_kind, created_at, updated_at, version) VALUES (?, ?, ?, ?, ?, ?, ?, 1);",
        )
        .run("dup-b", "src-1", "task", "tgt-1", "task", base + 500, base + 500);

      storage.db.exec("PRAGMA user_version = 11;");
      const { join } = require("node:path");
      const { writeFileSync: wfs } = require("node:fs");
      const markerPath = join(workspace, ".trekoon", "migration-version");
      wfs(markerPath, JSON.stringify({ version: 11, userVersion: 11 }), "utf8");
      ...
      expect(migrationWarns.length).toBeGreaterThan(0);
      expect(migrationWarns[0]).toMatch(/removed 1 duplicate/iu);
    } finally {
      storage.close();
    }
  });
